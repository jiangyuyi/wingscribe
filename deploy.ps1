...
function Invoke-ConfigWizard {
    ...
    # Normalize user-provided paths for YAML (use forward slashes on all platforms)
    $normalizedSourceDir = $sourceDir.Replace('\', '/')
    ...
    $lines += "paths:"
    $lines += "  sources:"
    $lines += "    - path: ""$normalizedSourceDir"""
    $lines += "      recursive: true"
    $lines += "      enabled: true"
    $lines += "  output:"
    $lines += "    root_dir: ""$normalizedOutputDir"""
    ...
    $lines += "processing:"
    $lines += "  device: ""$device"""
    $lines += "  yolo_model: ""data/models/yolo26n.pt"""
    ...
    $lines += "  hf_mirror: ""https://hf-mirror.com"""
    $lines += "  local:"
