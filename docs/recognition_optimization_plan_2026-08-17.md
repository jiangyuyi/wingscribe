...
首版只实现 province/national。月度数据的首轮公共验证没有收益，后续生产设计不再引入月份；市级数据只有在来源、行政区映射和样本量都足够时再单独评估。
...
- 无地点或无先验数据时必须与纯视觉结果一致。

数据来源优先级：

1. 用户人工确认的历史照片，形成个人常见度统计。
2. 用户合法取得并导入的地区全年观测汇总。
...
- 已完成多指标 `QualityEvaluator`，并以 `legacy_reject`、`score_only`、`disabled` 三种模式接入流水线。默认 `legacy_reject` 保持旧行为；新处理照片会保存综合质量分和完整子指标，但尚不参与识别排序。
- 已完成独立 `LocationResolver`、行政区 CSV/用户别名 YAML 格式、歧义拒绝、词典版本缓存和只读预览 API；当前不接入生产识别，下一步需确认完整行政区数据的来源与许可证并执行预览验证。
- 已增加人工标签来源与确认时间，只有明确经过人工修改/确认的照片才可作为未来 prototype 样本；历史标签不做不可靠回填。
- 连拍排序暂缓：当前只保存日期，没有 EXIF 秒级拍摄时间。下一步应先只读采集 `DateTimeOriginal`，不能只靠文件名连续自动分组。
