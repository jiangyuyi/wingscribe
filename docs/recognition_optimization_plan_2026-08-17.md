...
- 已完成公共评测清单、CUB 适配器、确定性分层抽样、报告比较和无标签一致性指标；CUB 官方数据包已通过镜像取得并以官方 MD5 校验，完整 test split 的 BioCLIP2/BioCLIP 2.5 实测已经完成。
...
- CUB 官方 test split（5,794 张、官方 bbox、200 类）上，BioCLIP 2.5 Top-1/Top-5 为 90.65%/97.98%，BioCLIP2 为 88.61%/97.38%；Top-1 提升 2.04 个百分点，229 张改善、111 张退化，配对精确检验 `p=1.46e-10`。2.5 平均耗时约 39.8 ms/张，BioCLIP2 约 26.6 ms/张；详细边界见独立结果文档。
- 当前两路 multi-crop 在 200 类分层样本上没有优于单 bbox，不接入默认流程；后续若继续实验，应先调整视野和融合权重，而不是直接扩大运行规模。
...
- BioCLIP 2.5 已有公共集收益证据，但 CUB 以北美鸟种为主且官方提示可能与 ImageNet/预训练图片重叠，仍不足以代表中国野外照片；生产默认继续使用 BioCLIP2，下一步优先补充 iNaturalist 中国鸟类固定子集。
- 已完成 iNaturalist 中国鸟类冻结子集：完整扫描 17,428 条许可兼容观测，从 924 个候选物种中确定性抽取 600 张/600 种并校验图片和 SHA-256。BioCLIP2/2.5 整图 Top-1 为 66.33%/67.67%，差值 1.33 个百分点但配对检验 `p=0.516`；2.5 平均耗时约 2.44 倍、峰值 reserved 显存约 5.53 GB，因此生产默认保持 BioCLIP2。
- iNaturalist 清单中 2022-2025 年的 450 张敏感性分析结论相同（Top-1 增量 1.11 个百分点，`p=0.661`），但仍不能完全排除较新模型训练数据重叠。阶段 0 下一步转向任意本地目录的无标签影子比较，以及在冻结清单上验证可审计的地点/月度软先验。
